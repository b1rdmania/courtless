...
const IntakeFlow = () => {
  ...
  // 'new' — document-first flow. 'manual' — legacy 7-step fallback.
  ...
  const canProceed = () => {
    if (mode === 'new') {
      // Step 1: upload — need at least one file
      if (step === 1) return evidence.length > 0;
      // Step 2: review pre-fill — title + amount + description must be present
      if (step === 2) {
        return form.title.trim().length > 2
          && form.amount !== ''
          && form.dispute_description.trim().length >= 200;
      }
      // Step 3: outcome — textarea
      if (step === 3) return form.desired_outcome.trim().length >= 20;
      // Step 4: honest take — radio always has a default; steelman is optional
  ...
  // ----- Prefill: called on step 1 -> step 2 of the new flow -----
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === 'new' && pendingId) {
        // Files already staged on the backend — just reference them.
        body.append('pending_id', pendingId);
      } else {
        // Fallback: send files with the submit.
        evidence.forEach(ev => {
          body.append('files', ev.file);
          body.append('file_types', ev.upload_type);
          body.append('file_labels', ev.label);
          ...
          <>
            {mode === 'new' && (
              <>
                ...
                {step === 4 && <StepHonestTake form={form} update={update} />}
                {step === 5 && (
                  <StepSubmit
                    form={form}
                    update={update}
            ...
            {mode === 'manual' && (
              <>
                ...
                {step === 6 && (
                  <StepEvidence evidence={evidence} setEvidence={setEvidence} />
                )}
                {step === 7 && (
...
const NavButtons = ({ step, total, canProceed, onBack, onNext, onSubmit, nextLabel }) => {
...
// ---- Step 1: upload first ----
const StepUploadFirst = ({ evidence, setEvidence, onSkip }) => {
  ...
  return (
    <div>
      <h2 style={questionTitle}>Drop your evidence in. We'll read it.</h2>
      <p style={questionHelper}>
        Emails, contracts, invoices, messages, photos — drop them here and we'll pull
        the facts out, summarise it, and walk you through the rest. Beats typing a
        3,000-word essay from memory.
      </p>
            ...
            >
              <div>
                ...
                <input
                  ...
                  placeholder="Label (optional) — e.g. 'signed contract', 'final invoice'"
                  style={{
      ...
      <div style={{ marginTop: '28px', textAlign: 'center' }}>
        <button
          onClick={onSkip}
          style={{
            fontSize: '13px', color: 'rgba(235, 235, 245, 0.55)',
            background: 'none', border: 'none', cursor: 'pointer', fontFamily,
...
// ---- Step 2: review AI pre-fill ----
const StepReviewPrefill = ({ form, update, keyEvidenceSummary }) => (
  <div>
    <h2 style={questionTitle}>We read your docs. Here's what we got.</h2>
    <p style={questionHelper}>
      This is a starting point, not the final word. Edit anything that's wrong, fill in
      anything we missed. You know the story — we just scaffold it.
    </p>
    ...
    <div style={{ marginBottom: '24px' }}>
      <label style={labelStyle}>Other party's name</label>
      <input
        type="text"
    ...
    <div style={{ marginBottom: '24px' }}>
      <label style={labelStyle}>What happened, in your words</label>
      <textarea
      ...
      <div style={{ fontSize: '12px', color: 'rgba(235, 235, 245, 0.4)', marginTop: '6px' }}>
        {form.dispute_description.trim().length} characters
      </div>
    </div>
    ...
    {Array.isArray(keyEvidenceSummary) && keyEvidenceSummary.length > 0 && (
      <div style={{
        ...
        <div style={{
          ...
          color: 'rgba(235, 235, 245, 0.5)', fontWeight: 600, marginBottom: '12px',
        }}>
          What we pulled out of your documents
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {keyEvidenceSummary.map((item, i) => (
            <div key={i} style={{ fontSize: '13px', color: 'rgba(235, 235, 245, 0.72)', lineHeight: 1.55 }}>
              <span style={{ color: '#0A84FF', fontWeight: 600 }}>{item.filename}</span>
              <span style={{ color: 'rgba(235, 235, 245, 0.45)' }}> — </span>
              {item.summarises_as}
            </div>
          ))}
        </div>
...
// ---- Step 3: outcome ---- (shared with fallback)
const StepOutcome = ({ form, update }) => (
  <div>
    <h2 style={questionTitle}>What does a good outcome look like for you?</h2>
    <p style={questionHelper}>
      Full amount back? A partial refund and an apology? A specific action they need to
      take? Knowing what you actually want sharpens the whole audit.
    </p>
    <textarea
    ...
    <div style={{ fontSize: '12px', color: 'rgba(235, 235, 245, 0.45)', marginTop: '8px', textAlign: 'right' }}>
      {form.desired_outcome.trim().length} characters
    </div>
  </div>
);

// ---- Step 4: honest take (new) ----
const StepHonestTake = ({ form, update }) => (
  <div>
    <h2 style={questionTitle}>Your honest take.</h2>
    <p style={questionHelper}>
      Disputes rarely have one clean villain. Thinking about your own role now makes the
      audit sharper — and your steelman for the other side better.
    </p>

    <div style={{ marginBottom: '28px' }}>
      <div style={labelStyle}>Do you think you bear any responsibility?</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {[
...
// ---- Step 5: submit + consent (new) ----
const StepSubmit = ({ form, update, evidence }) => (
  <div>
...
// ---- Step 7: review + consent (fallback) ----
